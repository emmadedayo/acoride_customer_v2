...
class BillProvider{
  ...
  Future<List> getBills(String type,token) async {
    Response? response = await RestApi.getDataFromServer('${ApiUrl.getBill}/$type', {}, method:
  ...
  Future<List> getVariation(String type,token) async {
    Response? response = await RestApi.getDataFromServer('${ApiUrl.getVariation}/$type', {}, method:
    HttpMethod.GET_WITH_AUTH,token:token);
    return RestApi.getListFromResponse(response);
  }
